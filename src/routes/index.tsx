...
import {
  Star, Zap, Truck, ShieldCheck,
  CreditCard, Banknote, Instagram, MessageCircle, ArrowRight, Check, UserPlus,
} from "lucide-react";
...
function Index() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <Footer footer={s.footer} contact={s.contact} />
    </div>
  );
}

/* ---------------- HERO ---------------- */
function Hero({ hero, stats, contact }: { hero: typeof DEFAULTS.hero; stats: typeof DEFAULTS.stats; contact: typeof DEFAULTS.contact }) {
  return (
      ...
      <div className="relative mx-auto grid max-w-7xl gap-12 px-5 pb-20 pt-16 md:grid-cols-12 md:pt-24">
        <div className="md:col-span-6 lg:col-span-7">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <a href={contact.whatsapp} target="_blank" rel="noreferrer"
              ...
              {hero.cta_primary} <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" />
            </a>
            <a href="#categorias" className="inline-flex items-center gap-2 rounded-sm border border-border bg-surface px-6 py-4 text-sm font-bold uppercase tracking-widest text-foreground transition hover:border-primary">
              {hero.cta_secondary}
            </a>
          </div>
          <dl className="mt-12 grid grid-cols-3 gap-6 border-t border-border pt-8">
            {stats.map((stat) => (
              <div key={stat.v}>
                ...
                <dd className="mt-1 text-xs uppercase tracking-wider text-muted-foreground">{stat.v}</dd>
              </div>
            ))}
          </dl>
        </div>
        <div className="md:col-span-6 lg:col-span-5">
          <div className="relative">
            ...
            <img src={heroProducts} alt="Fones, smartwatch e camisa em destaque" width={1600} height={1280}
            ...
            <div className="absolute -bottom-6 -left-6 rounded-sm border border-border bg-background px-5 py-4 shadow-2xl">
              ...
              <div className="mt-1 text-xs uppercase tracking-wider text-muted-foreground">Avaliação dos clientes</div>
            </div>
          </div>
        </div>
      </div>
...
/* ---------------- CATEGORIAS ---------------- */
const CATEGORIES = [
  { name: "Camisas", slug: "camisas", img: catCamisas, items: "Times, retrôs e casuais" },
  { name: "Fones", slug: "fones", img: catFones, items: "Bluetooth, in-ear, gamer" },
  { name: "Smartwatch", slug: "smartwatch", img: catSmartwatch, items: "Modelos novos e clássicos" },
  { name: "Eletrônicos", slug: "eletronicos", img: catEletronicos, items: "Cabos, caixinhas, gadgets" },
];

function Categorias() {
  return (
    <section id="categorias" className="mx-auto max-w-7xl px-5 py-24">
      <SectionHead eyebrow="O catálogo" title={<>Categorias <span className="text-primary">destaque</span></>} />
      <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {CATEGORIES.map((c) => (
          <Link key={c.slug} to="/c/$slug" params={{ slug: c.slug }}
            ...
            <div className="flex items-center justify-between border-t border-border p-5">
              <div>
                ...
                <div className="text-xs uppercase tracking-wider text-muted-foreground">{c.items}</div>
              </div>
...
/* ---------------- DESTAQUES ---------------- */
const HIGHLIGHTS = [
  { tag: "Mais pedido", title: "Fone Bluetooth Premium", line: "Bateria de alta duração com som imersivo.", img: catFones },
  { tag: "Tendência", title: "Smartwatch Série Y", line: "Notificações, monitor cardíaco e GPS.", img: catSmartwatch },
  { tag: "Coleção", title: "Camisas Oficiais", line: "Times nacionais e europeus, várias temporadas.", img: catCamisas },
];
    ...
    <section id="destaques" className="border-t border-border bg-surface/40">
      <div className="mx-auto max-w-7xl px-5 py-24">
        <SectionHead
          ...
          title={<>O que está <span className="text-primary">saindo</span> da loja</>}
          right={<a href={contact.instagram} target="_blank" rel="noreferrer"
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {HIGHLIGHTS.map((h) => (
            <article key={h.title} className="group relative overflow-hidden rounded-sm border border-border bg-background">
              <div className="aspect-[4/5] overflow-hidden">
                <img src={h.img} alt={h.title} loading="lazy" width={800} height={1000}
                  className="h-full w-full object-cover transition duration-700 group-hover:scale-105" />
              </div>
...
function ComoFunciona() {
  const steps = [
    { n: "01", t: "Escolha o produto", d: "Veja as categorias ou peça uma indicação direto pelo WhatsApp." },
  ...
  return (
    <section className="mx-auto max-w-7xl px-5 py-24">
      <SectionHead eyebrow="Como funciona" title={<>3 passos <span className="text-primary">e pronto</span></>} />
  ...
  return (
    <section id="entrega" className="relative overflow-hidden border-y border-border bg-surface/40">
      <div className="mx-auto grid max-w-7xl gap-12 px-5 py-24 md:grid-cols-2 md:items-center">
        <div>
          <div className="display text-xs uppercase tracking-[0.3em] text-primary">Pediu, chegou</div>
          <h2 className="display mt-3 text-5xl md:text-6xl">Entrega por <span className="text-primary">motoboy</span></h2>
          <p className="mt-4 max-w-md text-muted-foreground">Receba na sua casa, no trabalho ou onde você estiver.</p>
          <ul className="mt-6 grid gap-3 text-sm">
            {["Atendimento humano via WhatsApp","Embalagem segura para eletrônicos","Garantia em todos os produtos","Suporte pós-venda dedicado"].map((b) => (
              <li key={b} className="flex items-center gap-3">
                <span className="grid h-6 w-6 place-items-center rounded-full bg-primary text-primary-foreground"><Check className="h-3.5 w-3.5" /></span>{b}
              </li>
            ))}
          </ul>
          <div className="mt-8">
            <div className="text-xs uppercase tracking-widest text-muted-foreground">Formas de pagamento</div>
            <div className="mt-3 flex flex-wrap gap-2">
              {pay.map((p) => (
                <span key={p.t} className="inline-flex items-center gap-2 rounded-sm border border-border bg-background px-4 py-2 text-sm font-bold">
                  {p.i} {p.t}
                </span>
              ))}
            </div>
        ...
        <div className="relative">
          <img src={motoboy} alt="Entrega por motoboy" loading="lazy" width={1200} height={800} className="w-full rounded-sm border border-border" />
        </div>
      </div>
    </section>
  );
    ...
    return (
      <section className="border-t border-border bg-surface/40">
        <div className="mx-auto max-w-3xl px-5 py-16 text-center">
          ...
          <p className="mt-2 text-muted-foreground">Acesse sua área pra acompanhar pedidos e pedir trocas.</p>
          <Link to="/minha-conta" className="mt-6 inline-block rounded-sm bg-primary px-6 py-3 text-sm font-bold uppercase tracking-wider text-primary-foreground">
            Minha conta
          </Link>
        </div>
      </section>
  ...
  return (
    <section id="cadastro" className="border-t border-border">
      <div className="mx-auto grid max-w-6xl gap-10 px-5 py-24 md:grid-cols-2 md:items-center">
        <div>
          <div className="display text-xs uppercase tracking-[0.3em] text-primary">Crie sua conta</div>
          <h2 className="display mt-3 text-5xl">Compre mais <span className="text-primary">rápido</span>.</h2>
          <ul className="mt-6 space-y-3 text-sm">
            {["Histórico de pedidos sempre à mão","Solicite trocas em 1 clique","Checkout sem digitar tudo de novo","Status do pedido em tempo real"].map((b) => (
              <li key={b} className="flex items-center gap-3">
                <span className="grid h-6 w-6 place-items-center rounded-full bg-primary text-primary-foreground"><Check className="h-3.5 w-3.5" /></span>{b}
              </li>
            ))}
        ...
        <form onSubmit={submit} className="space-y-3 rounded-sm border border-border bg-surface p-6">
          <h3 className="display text-2xl">Cadastro rápido</h3>
          <input required placeholder="Seu nome completo" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
          ...
          <input required type="email" placeholder="seu@email.com" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })}
            className="w-full rounded-sm border border-border bg-background px-4 py-3 text-sm" />
          <input required type="password" minLength={6} placeholder="Senha (mín. 6 caracteres)" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })}
            className="w-full rounded-sm border border-border bg-background px-4 py-3 text-sm" />
          <button disabled={loading} className="w-full rounded-sm bg-primary py-3 text-sm font-bold uppercase tracking-wider text-primary-foreground hover:brightness-110 disabled:opacity-60">
...
/* ---------------- DEPOIMENTOS ---------------- */
const REVIEWS = [
  { n: "Alice A.", r: "Comprei meu fone bluetooth e chegou no mesmo dia. Atendimento top.", t: "Cliente desde 2023" },
...
function Depoimentos() {
  return (
    <section className="mx-auto max-w-7xl px-5 py-24">
      <SectionHead eyebrow="Feedback real" title={<>Quem compra, <span className="text-primary">volta</span></>} />
      <div className="mt-12 grid gap-5 md:grid-cols-3">
        {REVIEWS.map((r) => (
          <figure key={r.n} className="flex flex-col rounded-sm border border-border bg-surface p-6">
  ...
  return (
    <section id="faq" className="border-y border-border bg-surface/40">
      <div className="mx-auto max-w-4xl px-5 py-24">
        <SectionHead eyebrow="Perguntas frequentes" title={<>Tirando suas <span className="text-primary">dúvidas</span></>} />
          ...
          {faq.map((f, i) => (
            <details key={i} className="group p-6 [&_summary::-webkit-details-marker]:hidden">
              <summary className="flex cursor-pointer items-center justify-between gap-4 font-bold uppercase tracking-wider">
                {f.q}
                <span className="display text-3xl text-primary transition group-open:rotate-45">+</span>
              </summary>
  ...
  return (
    <section className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-5xl px-5 py-28 text-center">
        <div className="display text-xs uppercase tracking-[0.3em] text-primary">faça seu pedido</div>
...
/* ---------------- FOOTER ---------------- */
function Footer({ footer, contact }: { footer: typeof DEFAULTS.footer; contact: typeof DEFAULTS.contact }) {
  return (
    <footer className="border-t border-border bg-background">
      <div className="mx-auto grid max-w-7xl gap-8 px-5 py-12 md:grid-cols-3">
        <div className="flex items-start gap-3">
          <img src={logoSt} alt="Logo Loja ST" className="h-12 w-12 object-contain" />
          <div>
        ...
        <div>
          ...
          <ul className="mt-3 space-y-1 text-sm">
            {CATEGORIES.map((c) => <li key={c.name}><Link to="/c/$slug" params={{ slug: c.slug }} className="hover:text-primary">{c.name}</Link></li>)}
          </ul>
        </div>
        <div>
          <div className="text-xs uppercase tracking-widest text-muted-foreground">Fale com a gente</div>
          <ul className="mt-3 space-y-2 text-sm">
            <li><a href={contact.instagram} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 hover:text-primary"><Instagram className="h-4 w-4" /> @alojast</a></li>
            <li><a href={contact.whatsapp} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 hover:text-primary"><MessageCircle className="h-4 w-4" /> WhatsApp</a></li>
            <li className="flex items-center gap-2 text-muted-foreground"><Truck className="h-4 w-4 text-primary" /> Entrega por motoboy</li>
            <li className="flex items-center gap-2 text-muted-foreground"><ShieldCheck className="h-4 w-4 text-primary" /> Garantia em todos os produtos</li>
          </ul>
        </div>
      </div>
